...
export default function RegisterPassword(props: {password:string, setPassword:React.Dispatch<React.SetStateAction<string>>}){
    ...
    return(
        <div className="p-1 bg-none flex justify-evanly items-center gap-4 border rounded border-slate-600 mt-1">
            ...
            <input className="bg-transparent outline-0 text-white " value={password} onChange={(e) => setPassword(e.target.value)} type="password" name="login_password" id="login_password" placeholder="Enter the password"/>
        </div>
    )
}