...
export default function TaskEditForm(props: {name:string, description:string, date_start:string, date_end:string, setIsEditing: React.Dispatch<React.SetStateAction<boolean>>,  editTask: (taskName: string, taskDescryption: string, dateStart: string, dateEnd: string, taskId: string) => Promise<void>, task_id:string}){
    const { name, description, date_start, date_end, setIsEditing, editTask, task_id} = props
    ...
    return (
        <form className="flex flex-col gap-2 ">
          ...
          <ButtonCancle setState={setIsEditing}/>
        </form>
    )
}