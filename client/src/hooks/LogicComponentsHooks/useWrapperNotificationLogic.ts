...
export const useWrapperNotificationLogic = () => {
  const [notifiactions, setNotifications] = useState<INotyfiaction[]>([]);
  ...
  const builderNotyfications = (tasks: ITask[]) => {
    ...
    setNotifications(newExtractedList);
  };
